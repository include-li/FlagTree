...
# Provenance: results/gluon/gluon_baseline/01_grouped_m128_n4096_g32x128_w4.json
...
def grouped_w4_store(out_ptr, in_ptr, w_ptr, w_scale_ptr, N, eps, GROUP_SIZE: gl.constexpr, NUM_GROUPS: gl.constexpr):
    """HCU helper SHA256: 0a24f7f7d698bdb8c366a4bbdf390f5a323c1b66691e8385072f7080ddc45c2c"""
    pid = __h81.freeze(gl.program_id(0))
    groups = __h81.freeze(__h81.arange(0, __h81.to_auto(NUM_GROUPS), layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0])))
    cols = __h81.freeze(__h81.arange(0, __h81.to_auto(GROUP_SIZE)))
    offsets = __h81.freeze(__h81.expand_dims(__h81.to_auto(groups), 1) * __h81.to_auto(GROUP_SIZE) + __h81.expand_dims(__h81.to_auto(cols), 0), _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))
    mask = __h81.freeze(__h81.compare(__h81.to_auto(offsets), __h81.to_auto(N), 'lt'), _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))
    x = __h81.freeze(__h81.load(__h81.to_auto(in_ptr) + __h81.to_auto(pid) * __h81.to_auto(N) + __h81.to_auto(offsets), mask=__h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0])).to(gl.float32), _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))
    ...
    w = __h81.freeze(__h81.load(__h81.to_auto(w_ptr) + __h81.to_auto(offsets), mask=__h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0])).to(gl.float32), _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))
    w_scale = __h81.freeze(__h81.expand_dims(__h81.load(__h81.to_auto(w_scale_ptr) + __h81.to_auto(groups), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))).to(gl.float32), 1), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0])))
    y = __h81.freeze(__h81.to_auto(x) * __h81.to_auto(rrms) * __h81.to_auto(w) * __h81.to_auto(w_scale), _layout=gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0]))
