...
# Original: /public/home/scnethpc2653/lhd/env/projects/hard81_20260910_phase2/src/FlagGems-vllm/src/flaggems_vllm/ops/mhc/mhc_pre.py
# Provenance: results/gluon/official/h2560_b1024_w4_s1_weuNone.json
...
def _mhc_pre_fused_kernel_hc_mult_4_impl_2903e7b1dd(gemm_out_ptr, hc_scale_ptr, hc_base_ptr, residual_ptr, post_mix_ptr, comb_mix_ptr, layer_input_ptr, num_tokens, num_tokens_bucket, res_stride_n, res_stride_i, res_stride_h, li_stride_n, li_stride_h, hidden_size, hc_hidden_size, rms_eps: gl.constexpr, hc_pre_eps: gl.constexpr, hc_sinkhorn_eps: gl.constexpr, hc_post_mult_value: gl.constexpr, sinkhorn_repeat: gl.constexpr, HC_MULT3: gl.constexpr, BLOCK_H: gl.constexpr):
    ...
    for k in gl.static_range(4):
        ...
        for h_start in range(0, __h81.to_auto(hidden_size), __h81.to_auto(BLOCK_H)):
            h_offsets = __h81.freeze(__h81.to_auto(h_start) + __h81.arange(0, __h81.to_auto(BLOCK_H), layout=gl.BlockedLayout([4], [64], [4], [0])))
            h_mask = __h81.freeze(__h81.compare(__h81.to_auto(h_offsets), __h81.to_auto(hidden_size), 'lt'))
            v = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(head_base) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32))
            sq = __h81.freeze(__h81.to_auto(sq) + __h81.to_auto(gl.sum(__h81.concrete(__h81.to_auto(v) * __h81.to_auto(v), _layout=gl.BlockedLayout([1], [64], [4], [0])))))
    ...
    for h_start in range(0, __h81.to_auto(hidden_size), __h81.to_auto(BLOCK_H)):
        ...
        h_mask = __h81.freeze(__h81.compare(__h81.to_auto(h_offsets), __h81.to_auto(hidden_size), 'lt'))
        r0 = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(res_base) + 0 * __h81.to_auto(res_stride_i) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32))
        r1 = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(res_base) + 1 * __h81.to_auto(res_stride_i) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32))
        acc = __h81.freeze(__h81.to_auto(pre_mix_0) * __h81.to_auto(r0) + __h81.to_auto(pre_mix_1) * __h81.to_auto(r1))
        r2 = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(res_base) + 2 * __h81.to_auto(res_stride_i) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32))
        r3 = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(res_base) + 3 * __h81.to_auto(res_stride_i) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32))
