...
# Provenance: results/gluon/gluon_baseline/small_m64_n4096_bfloat16_t4096_w4.json
...
def add_rms_norm_kernel_053039aa3c(out_ptr, in_ptr1, in_ptr2, w_ptr, y_stride_r, y_stride_c, x1_stride_r, x1_stride_c, x2_stride_r, x2_stride_c, N, eps, BLOCK_SIZE: gl.constexpr):
    ...
    mask = __h81.freeze(__h81.compare(__h81.arange(0, __h81.to_auto(BLOCK_SIZE), layout=gl.BlockedLayout([8], [64], [4], [0])), __h81.to_auto(N), 'lt'), _layout=gl.BlockedLayout([8], [64], [4], [0]))
    cols = __h81.freeze(__h81.arange(0, __h81.to_auto(BLOCK_SIZE)))
    x1 = __h81.freeze(__h81.load(__h81.to_auto(in_ptr1) + __h81.to_auto(cols) * __h81.to_auto(x1_stride_c), __h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([8], [64], [4], [0])).to(__h81.to_auto(cdtype)), _layout=gl.BlockedLayout([8], [64], [4], [0]))
    x2 = __h81.freeze(__h81.load(__h81.to_auto(in_ptr2) + __h81.to_auto(cols) * __h81.to_auto(x2_stride_c), __h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([8], [64], [4], [0])).to(__h81.to_auto(cdtype)), _layout=gl.BlockedLayout([8], [64], [4], [0]))
    x = __h81.freeze(__h81.to_auto(x1) + __h81.to_auto(x2), _layout=gl.BlockedLayout([8], [64], [4], [0]))
    var = __h81.freeze(__h81.to_auto(gl.sum(__h81.concrete(__h81.to_auto(x) * __h81.to_auto(x), _layout=gl.BlockedLayout([8], [64], [4], [0])), axis=0)) / __h81.to_auto(N), _layout=gl.BlockedLayout([8], [64], [4], [0]))
    rrms = __h81.freeze(1 / gl.sqrt(__h81.to_auto(var) + __h81.to_auto(eps)))
    w = __h81.freeze(__h81.load(__h81.to_auto(w_ptr) + __h81.arange(0, __h81.to_auto(BLOCK_SIZE)), mask=__h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([8], [64], [4], [0])), _layout=gl.BlockedLayout([8], [64], [4], [0]))
    ...
    __h81.store(__h81.to_auto(out_ptr) + __h81.to_auto(cols) * __h81.to_auto(y_stride_c), __h81.to_auto(y), mask=__h81.to_auto(mask), _layout=gl.BlockedLayout([8], [64], [4], [0]))
...
def sum_f3120590e3(input, axis=None, keep_dims=False, dtype: gl.constexpr=None):
    ...
    return __h81.to_auto(gl.reduce(__h81.concrete(__h81.to_auto(input), _layout=gl.BlockedLayout([8], [64], [4], [0])), __h81.to_auto(axis), _sum_combine_9669e8607d, keep_dims=__h81.to_auto(keep_dims)))
