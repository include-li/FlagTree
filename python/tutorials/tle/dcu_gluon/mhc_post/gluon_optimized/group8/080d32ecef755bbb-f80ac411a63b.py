...
def mhc_post_kernel_generic_aa7d23b7f7(a_ptr, b_ptr, c_ptr, d_ptr, out_ptr, H: gl.constexpr, HC: gl.constexpr, BLOCK_H: gl.constexpr):
    ...
    h_off = __h81.freeze(__h81.to_auto(pid_h) * __h81.to_auto(BLOCK_H) + __h81.arange(0, __h81.to_auto(BLOCK_H), layout=gl.BlockedLayout([8], [64], [8], [0])), _layout=gl.BlockedLayout([8], [64], [8], [0]))
    h_mask = __h81.freeze(__h81.compare(__h81.to_auto(h_off), __h81.to_auto(H), 'lt'), _layout=gl.BlockedLayout([8], [64], [8], [0]))
    ...
    d_vals = __h81.freeze(__h81.load(__h81.to_auto(d_ptr) + __h81.to_auto(d_base) + __h81.to_auto(h_off), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([8], [64], [8], [0])).to(gl.float32), _layout=gl.BlockedLayout([8], [64], [8], [0]))
    c_i = __h81.freeze(__h81.load(__h81.to_auto(c_ptr) + __h81.to_auto(c_base) + __h81.to_auto(pid_i)).to(gl.float32))
    acc = __h81.freeze(__h81.to_auto(c_i) * __h81.to_auto(d_vals), _layout=gl.BlockedLayout([8], [64], [8], [0]))
    for j in gl.static_range(0, __h81.to_auto(HC)):
        ...
        b_j = __h81.freeze(__h81.load(__h81.to_auto(b_ptr) + __h81.to_auto(b_base) + __h81.to_auto(j) * __h81.to_auto(H) + __h81.to_auto(h_off), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([8], [64], [8], [0])).to(gl.float32), _layout=gl.BlockedLayout([8], [64], [8], [0]))
        acc = __h81.freeze(__h81.to_auto(acc) + __h81.to_auto(a_ji) * __h81.to_auto(b_j), _layout=gl.BlockedLayout([8], [64], [8], [0]))
    __h81.store(__h81.to_auto(out_ptr) + __h81.to_auto(out_base) + __h81.to_auto(h_off), __h81.cast(acc, gl.bfloat16, _layout=gl.BlockedLayout([8], [64], [8], [0])), mask=__h81.to_auto(h_mask), _layout=gl.BlockedLayout([8], [64], [8], [0]))
