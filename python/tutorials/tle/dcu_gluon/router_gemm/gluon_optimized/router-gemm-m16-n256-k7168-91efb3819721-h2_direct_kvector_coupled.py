...
# Provenance: results/gluon/gluon_baseline/router-gemm-m16-n256-k7168-91efb3819721-409a50ed425e3c59.json
...
def mm_kernel_general_9368d87839(A, B, C, M, N, K, stride_am, stride_ak, stride_bk, stride_bn, stride_cm, stride_cn, BLOCK_M: gl.constexpr, BLOCK_N: gl.constexpr, BLOCK_K: gl.constexpr, GROUP_M: gl.constexpr, IS_FP64: gl.constexpr=False):
    ...
    rm = __h81.freeze(__h81.to_auto(pid_m) * __h81.to_auto(BLOCK_M) + __h81.arange(0, __h81.to_auto(BLOCK_M), layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [8, 8], [2, 1], [1, 0]))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [8, 8], [2, 1], [1, 0])))
    rn = __h81.freeze(__h81.to_auto(pid_n) * __h81.to_auto(BLOCK_N) + __h81.arange(0, __h81.to_auto(BLOCK_N), layout=gl.SliceLayout(0, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1]))), _layout=gl.SliceLayout(0, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))
    ram = __h81.freeze(gl.max_contiguous(gl.multiple_of(__h81.to_auto(rm) % __h81.to_auto(M), __h81.to_auto(BLOCK_M)), __h81.to_auto(BLOCK_M)).to(gl.int64), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [8, 8], [2, 1], [1, 0])))
    rbn = __h81.freeze(gl.max_contiguous(gl.multiple_of(__h81.to_auto(rn) % __h81.to_auto(N), __h81.to_auto(BLOCK_N)), __h81.to_auto(BLOCK_N)).to(gl.int64), _layout=gl.SliceLayout(0, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))
    rm = __h81.freeze(__h81.cast(rm, gl.int64), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [8, 8], [2, 1], [1, 0])))
    rn = __h81.freeze(__h81.cast(rn, gl.int64), _layout=gl.SliceLayout(0, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))
    ...
    for start_k in range(0, __h81.to_auto(prev_multiple), __h81.to_auto(BLOCK_K)):
        ...
        rk_b = __h81.freeze((__h81.to_auto(start_k) + __h81.arange(0, __h81.to_auto(BLOCK_K), layout=gl.SliceLayout(1, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))).to(gl.int64), _layout=gl.SliceLayout(1, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))
        b = __h81.freeze(__h81.load(__h81.to_auto(B) + (__h81.expand_dims(__h81.to_auto(rk_b), 1) * __h81.to_auto(stride_bk) + __h81.expand_dims(__h81.to_auto(rbn), 0) * __h81.to_auto(stride_bn)), _layout=gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])), _layout=gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1]))
        if __h81.compare(__h81.to_auto(a).dtype, __h81.to_auto(b).dtype, 'ne'):
            ...
            b = __h81.freeze(__h81.cast(b, __h81.pointer_element_type(__h81.to_auto(C), True)), _layout=gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1]))
        if __h81.to_auto(IS_FP64):
    ...
    rk_b = __h81.freeze((__h81.to_auto(prev_multiple) + __h81.arange(0, __h81.to_auto(BLOCK_K), layout=gl.SliceLayout(1, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))).to(gl.int64), _layout=gl.SliceLayout(1, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))
    mask_k_b = __h81.freeze(__h81.compare(__h81.to_auto(rk_b), __h81.to_auto(K), 'lt'), _layout=gl.SliceLayout(1, gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])))
    b = __h81.freeze(__h81.load(__h81.to_auto(B) + (__h81.expand_dims(__h81.to_auto(rk_b), 1) * __h81.to_auto(stride_bk) + __h81.expand_dims(__h81.to_auto(rbn), 0) * __h81.to_auto(stride_bn)), mask=__h81.expand_dims(__h81.to_auto(mask_k_b), 1), other=0.0, _layout=gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1])), _layout=gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1]))
    if __h81.compare(__h81.to_auto(a).dtype, __h81.to_auto(b).dtype, 'ne'):
        a = __h81.freeze(__h81.cast(a, __h81.pointer_element_type(__h81.to_auto(C), True)), _layout=gl.BlockedLayout([1, 8], [8, 8], [2, 1], [1, 0]))
        b = __h81.freeze(__h81.cast(b, __h81.pointer_element_type(__h81.to_auto(C), True)), _layout=gl.BlockedLayout([8, 1], [8, 8], [1, 2], [0, 1]))
    if __h81.to_auto(IS_FP64):
