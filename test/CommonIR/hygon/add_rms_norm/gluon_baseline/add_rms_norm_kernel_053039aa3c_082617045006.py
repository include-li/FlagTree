...
# Provenance: results/gluon/07/add_rms_norm_kernel_053039aa3c_082617045006.json
...
def add_rms_norm_kernel_053039aa3c(out_ptr, in_ptr1, in_ptr2, w_ptr, y_stride_r, y_stride_c, x1_stride_r, x1_stride_c, x2_stride_r, x2_stride_c, N, eps, BLOCK_SIZE: gl.constexpr):
    ...
    x1 = __h81.freeze(__h81.load(__h81.to_auto(in_ptr1) + __h81.to_auto(cols) * __h81.to_auto(x1_stride_c), __h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([1], [64], [4], [0])).to(__h81.to_auto(cdtype)), _layout=gl.BlockedLayout([1], [64], [4], [0]))
    x2 = __h81.freeze(__h81.load(__h81.to_auto(in_ptr2) + __h81.to_auto(cols) * __h81.to_auto(x2_stride_c), __h81.to_auto(mask), other=0.0, _layout=gl.BlockedLayout([1], [64], [4], [0])).to(__h81.to_auto(cdtype)), _layout=gl.BlockedLayout([1], [64], [4], [0]))
    x = __h81.freeze(__h81.to_auto(x1) + __h81.to_auto(x2), _layout=gl.BlockedLayout([1], [64], [4], [0]))
    ...
    y = __h81.freeze((__h81.to_auto(x) * __h81.to_auto(rrms) * __h81.to_auto(w)).to(__h81.to_auto(cdtype)), _layout=gl.BlockedLayout([1], [64], [4], [0]))
