...
def baseline_slice_scale(o_ptr, positions_ptr, cos_sin_cache_ptr, fp8_ptr, scale_ptr, num_tokens, heads_per_group: gl.constexpr, o_stride_token, o_stride_head, cache_stride_pos, fp8_stride_group, fp8_stride_token, scale_stride_group, scale_stride_k, fp8_max: gl.constexpr, eps: gl.constexpr, QUANT_GROUP_SIZE: gl.constexpr, CHUNKS_PER_HEAD: gl.constexpr, ROPE_START: gl.constexpr, HALF_ROPE: gl.constexpr, TMA_ALIGNED_SCALES: gl.constexpr):
    ...
    if __h81.compare(__h81.to_auto(pid_token), __h81.to_auto(num_tokens), 'ge'):
        if __h81.to_auto(TMA_ALIGNED_SCALES):
            ...
            block_offsets = __h81.freeze(__h81.arange(0, __h81.to_auto(CHUNKS_PER_HEAD), layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0]))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0])))
            qb_indices = __h81.freeze(__h81.to_auto(qb_start) + __h81.to_auto(block_offsets), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0])))
            scale_addrs = __h81.freeze(__h81.to_auto(scale_ptr) + __h81.to_auto(g) * __h81.to_auto(scale_stride_group) + __h81.to_auto(pid_token) + __h81.to_auto(qb_indices) * __h81.to_auto(scale_stride_k), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0])))
            __h81.store(__h81.to_auto(scale_addrs), __h81.zeros((__h81.to_auto(CHUNKS_PER_HEAD),), dtype=gl.float32), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0])))
        return
    ...
    if __h81.to_auto(TMA_ALIGNED_SCALES):
        ...
        scale_addrs = __h81.freeze(__h81.to_auto(scale_ptr) + __h81.to_auto(g) * __h81.to_auto(scale_stride_group) + __h81.to_auto(pid_token) + __h81.to_auto(qb_indices) * __h81.to_auto(scale_stride_k), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0])))
        __h81.store(__h81.to_auto(scale_addrs), __h81.to_auto(scales), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 8], [4, 16], [1, 1], [1, 0])))
