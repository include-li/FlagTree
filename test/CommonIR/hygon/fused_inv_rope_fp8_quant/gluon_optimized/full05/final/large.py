...
def contig4_slice_scale(o_ptr, positions_ptr, cos_sin_cache_ptr, fp8_ptr, scale_ptr, num_tokens, heads_per_group: gl.constexpr, o_stride_token, o_stride_head, cache_stride_pos, fp8_stride_group, fp8_stride_token, scale_stride_group, scale_stride_k, fp8_max: gl.constexpr, eps: gl.constexpr, QUANT_GROUP_SIZE: gl.constexpr, CHUNKS_PER_HEAD: gl.constexpr, ROPE_START: gl.constexpr, HALF_ROPE: gl.constexpr, TMA_ALIGNED_SCALES: gl.constexpr):
    ...
    if __h81.compare(__h81.to_auto(pid_token), __h81.to_auto(num_tokens), 'ge'):
        if __h81.to_auto(TMA_ALIGNED_SCALES):
            ...
            block_offsets = __h81.freeze(__h81.arange(0, __h81.to_auto(CHUNKS_PER_HEAD), layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0]))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
            qb_indices = __h81.freeze(__h81.to_auto(qb_start) + __h81.to_auto(block_offsets), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
            scale_addrs = __h81.freeze(__h81.to_auto(scale_ptr) + __h81.to_auto(g) * __h81.to_auto(scale_stride_group) + __h81.to_auto(pid_token) + __h81.to_auto(qb_indices) * __h81.to_auto(scale_stride_k), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
            __h81.store(__h81.to_auto(scale_addrs), __h81.zeros((__h81.to_auto(CHUNKS_PER_HEAD),), dtype=gl.float32), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
        return
    ...
    offsets = __h81.freeze(__h81.arange(0, __h81.to_auto(HEAD_DIM), layout=gl.BlockedLayout([4], [64], [1], [0])), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    x = __h81.freeze(__h81.load(__h81.to_auto(input_base) + __h81.to_auto(offsets), _layout=gl.BlockedLayout([4], [64], [1], [0])).to(gl.float32), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    ...
    is_rope = __h81.freeze(__h81.compare(__h81.to_auto(offsets), __h81.to_auto(rope_abs_start), 'ge'), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    rope_local = __h81.freeze(__h81.to_auto(offsets) - __h81.to_auto(rope_abs_start), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    x_partner = __h81.freeze(__h81.load(__h81.to_auto(input_base) + (__h81.to_auto(offsets) ^ 1), mask=__h81.to_auto(is_rope), other=0.0, _layout=gl.BlockedLayout([4], [64], [1], [0])).to(gl.float32), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    cs_idx = __h81.freeze(gl.maximum(__h81.to_auto(rope_local) >> 1, 0), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    cos_v = __h81.freeze(__h81.load(__h81.to_auto(cache_base) + __h81.to_auto(cs_idx), mask=__h81.to_auto(is_rope), other=1.0, _layout=gl.BlockedLayout([4], [64], [1], [0])), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    sin_v = __h81.freeze(__h81.load(__h81.to_auto(cache_base) + __h81.to_auto(HALF_ROPE) + __h81.to_auto(cs_idx), mask=__h81.to_auto(is_rope), other=0.0, _layout=gl.BlockedLayout([4], [64], [1], [0])), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    x_add = __h81.freeze(__h81.to_auto(x) * __h81.to_auto(cos_v) + __h81.to_auto(x_partner) * __h81.to_auto(sin_v), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    x_sub = __h81.freeze(__h81.to_auto(x) * __h81.to_auto(cos_v) - __h81.to_auto(x_partner) * __h81.to_auto(sin_v), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    is_even = __h81.freeze(__h81.compare(__h81.to_auto(rope_local) & 1, 0, 'eq'), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    rotated = __h81.freeze(gl.where(__h81.to_auto(is_even), __h81.to_auto(x_add), __h81.to_auto(x_sub)), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    x = __h81.freeze(gl.where(__h81.to_auto(is_rope), __h81.to_auto(rotated), __h81.to_auto(x)), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    x_2d = __h81.freeze(gl.reshape(gl.abs(__h81.to_auto(x)), (__h81.to_auto(CHUNKS_PER_HEAD), __h81.to_auto(QUANT_GROUP_SIZE))), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    block_absmax = __h81.freeze(gl.maximum(__h81.to_auto(gl.max(__h81.concrete(__h81.to_auto(x_2d), _layout=gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])), axis=1)), __h81.to_auto(eps)), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
    scales = __h81.freeze(__h81.to_auto(block_absmax) * (1.0 / __h81.to_auto(fp8_max)), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
    if __h81.to_auto(TMA_ALIGNED_SCALES):
        scales = __h81.freeze(gl.exp2(gl.ceil(gl.log2(gl.maximum(gl.abs(__h81.to_auto(scales)), 1e-10)))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
    scales_exp = __h81.freeze(gl.reshape(_ext__fused_inv_rope_fp8_quant_per_head_625a5a92b8_tl.broadcast_to(gl.reshape(__h81.to_auto(scales), (__h81.to_auto(CHUNKS_PER_HEAD), 1)), (__h81.to_auto(CHUNKS_PER_HEAD), __h81.to_auto(QUANT_GROUP_SIZE))), (__h81.to_auto(HEAD_DIM),)), _layout=gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0]))
    x_quant = __h81.freeze(_ext__fused_inv_rope_fp8_quant_per_head_625a5a92b8_tl.clamp(__h81.to_auto(x) / __h81.to_auto(scales_exp), -__h81.to_auto(fp8_max), __h81.to_auto(fp8_max)).to(gl.float8e4nv), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    ...
    __h81.store(__h81.to_auto(fp8_base) + __h81.to_auto(offsets), __h81.to_auto(x_quant), _layout=gl.BlockedLayout([4], [64], [1], [0]))
    block_offsets = __h81.freeze(__h81.arange(0, __h81.to_auto(CHUNKS_PER_HEAD), layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0]))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
    qb_indices = __h81.freeze(__h81.to_auto(qb_start) + __h81.to_auto(block_offsets), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
    if __h81.to_auto(TMA_ALIGNED_SCALES):
        ...
        scale_addrs = __h81.freeze(__h81.to_auto(scale_ptr) + __h81.to_auto(g) * __h81.to_auto(scale_stride_group) + __h81.to_auto(pid_token) + __h81.to_auto(qb_indices) * __h81.to_auto(scale_stride_k), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
        __h81.store(__h81.to_auto(scale_addrs), __h81.to_auto(scales), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])))
...
def max_bc5e2fe0cf(input, axis=None, return_indices=False, return_indices_tie_break_left=True, keep_dims=False):
    ...
    if __h81.to_auto(return_indices):
        ...
        return __h81.to_auto(gl.reduce(__h81.concrete(__h81.to_auto(input), _layout=gl.BlockedLayout([1, 4], [2, 32], [1, 1], [1, 0])), __h81.to_auto(axis), _elementwise_max_dfab1c952e, keep_dims=__h81.to_auto(keep_dims)))
