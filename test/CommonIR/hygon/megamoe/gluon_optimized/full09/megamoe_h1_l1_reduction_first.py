...
# Provenance: results/gluon/gluon_baseline/mixtral-m16-h4096-i14336-e8-k2-ue8m0-_fp8_fp4_mega_moe_l1_kernel-5cb970117bbb46ee.json
...
def _fp8_fp4_mega_moe_l1_kernel_120b33aff9(x_ptr, x_scale_ptr, topk_idx_ptr, l1_w_ptr, l1_s_ptr, l1_out_ptr, M: gl.constexpr, H: gl.constexpr, I: gl.constexpr, TOP_K: gl.constexpr, stride_xm, stride_xh, stride_xsm, stride_xsg, stride_topkm, stride_topkk, stride_w_e, stride_w_n, stride_w_kp, stride_s_e, stride_s_n, stride_s_g, stride_out_m, stride_out_k, stride_out_n, BLOCK_N: gl.constexpr, BLOCK_K: gl.constexpr, SCALE_IS_UE8M0: gl.constexpr):
    ...
    n_offsets = __h81.freeze(__h81.to_auto(pid_n) * __h81.to_auto(BLOCK_N) + __h81.arange(0, __h81.to_auto(BLOCK_N), layout=gl.SliceLayout(0, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1]))), _layout=gl.SliceLayout(0, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
    k_offsets = __h81.freeze(__h81.arange(0, __h81.to_auto(BLOCK_K), layout=gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1]))), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
    ...
    acc = __h81.freeze(__h81.zeros([__h81.to_auto(BLOCK_N)], dtype=gl.float32), _layout=gl.SliceLayout(0, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
    for k0 in range(0, __h81.to_auto(H), __h81.to_auto(BLOCK_K)):
        ks = __h81.freeze(__h81.to_auto(k0) + __h81.to_auto(k_offsets), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
        x = __h81.freeze(__h81.load(__h81.to_auto(x_ptr) + __h81.to_auto(pid_m) * __h81.to_auto(stride_xm) + __h81.to_auto(ks) * __h81.to_auto(stride_xh), mask=__h81.compare(__h81.to_auto(ks), __h81.to_auto(H), 'lt'), other=0.0, _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1]))).to(gl.float32), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
        ...
        x = __h81.freeze(__h81.to_auto(x) * __h81.to_auto(x_scale), _layout=gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
        w = __h81.freeze(_load_fp4_weight_4cfbeeb897(l1_w_ptr, l1_s_ptr, expert, n_offsets, ks, stride_w_e, stride_w_n, stride_w_kp, stride_s_e, stride_s_n, stride_s_g, __h81.pin(2 * __h81.to_auto(I)), H, SCALE_IS_UE8M0), _layout=gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1]))
        acc = __h81.freeze(__h81.to_auto(acc) + __h81.to_auto(gl.sum(__h81.concrete(__h81.to_auto(w) * __h81.expand_dims(__h81.to_auto(x), 1), _layout=gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])), axis=0)), _layout=gl.SliceLayout(0, gl.BlockedLayout([1, 1], [32, 2], [1, 4], [0, 1])))
