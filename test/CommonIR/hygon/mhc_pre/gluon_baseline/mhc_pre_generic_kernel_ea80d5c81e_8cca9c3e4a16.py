...
# Provenance: results/gluon/16/mhc_pre_generic_kernel_ea80d5c81e_8cca9c3e4a16.json
...
def mhc_pre_generic_kernel_ea80d5c81e(gemm_out_ptr, hc_scale_ptr, hc_base_ptr, residual_ptr, post_mix_ptr, comb_mix_ptr, layer_input_ptr, num_tokens, num_tokens_bucket, res_stride_n, res_stride_i, res_stride_h, li_stride_n, li_stride_h, hidden_size, hc_hidden_size, rms_eps: gl.constexpr, hc_pre_eps: gl.constexpr, hc_sinkhorn_eps: gl.constexpr, hc_post_mult_value: gl.constexpr, sinkhorn_repeat: gl.constexpr, HC: gl.constexpr, BLOCK_H: gl.constexpr):
    ...
    sq = __h81.freeze(0.0, _layout=gl.BlockedLayout([1], [64], [4], [0]))
    for k in gl.static_range(__h81.to_auto(HC)):
        ...
        for h_start in range(0, __h81.to_auto(hidden_size), __h81.to_auto(BLOCK_H)):
            h_offsets = __h81.freeze(__h81.to_auto(h_start) + __h81.arange(0, __h81.to_auto(BLOCK_H), layout=gl.BlockedLayout([4], [64], [4], [0])), _layout=gl.BlockedLayout([4], [64], [4], [0]))
            h_mask = __h81.freeze(__h81.compare(__h81.to_auto(h_offsets), __h81.to_auto(hidden_size), 'lt'), _layout=gl.BlockedLayout([4], [64], [4], [0]))
            v = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(head_base) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32), _layout=gl.BlockedLayout([4], [64], [4], [0]))
            sq = __h81.freeze(__h81.to_auto(sq) + __h81.to_auto(gl.sum(__h81.concrete(__h81.to_auto(v) * __h81.to_auto(v), _layout=gl.BlockedLayout([1], [64], [4], [0])))), _layout=gl.BlockedLayout([1], [64], [4], [0]))
    ...
    for i in gl.static_range(__h81.to_auto(HC)):
        for j in gl.static_range(__h81.to_auto(HC)):
            ...
            v = __h81.freeze(__h81.load(__h81.to_auto(gemm_out_ptr) + __h81.to_auto(go_base) + __h81.to_auto(cb) + __h81.to_auto(idx)) * __h81.to_auto(rms_inv) * __h81.to_auto(scale_2) + __h81.load(__h81.to_auto(hc_base_ptr) + __h81.to_auto(cb) + __h81.to_auto(idx)), _layout=gl.BlockedLayout([4], [64], [4], [0]))
    ...
    for i in gl.static_range(__h81.to_auto(HC)):
        ...
        for j in gl.static_range(__h81.to_auto(HC)):
            v = __h81.freeze(__h81.load(__h81.to_auto(comb_mix_ptr) + __h81.to_auto(comb_base) + __h81.to_auto(i) * __h81.to_auto(HC) + __h81.to_auto(j)), _layout=gl.BlockedLayout([4], [64], [4], [0]))
    ...
    for _ in gl.static_range(__h81.to_auto(sinkhorn_repeat) - 1):
        ...
        for j in gl.static_range(__h81.to_auto(HC)):
            ...
            for i in gl.static_range(__h81.to_auto(HC)):
                ...
                __h81.store(__h81.to_auto(comb_mix_ptr) + __h81.to_auto(comb_base) + __h81.to_auto(i) * __h81.to_auto(HC) + __h81.to_auto(j), __h81.to_auto(v) * __h81.to_auto(inv_col_sum))
    for h_start in range(0, __h81.to_auto(hidden_size), __h81.to_auto(BLOCK_H)):
        h_offsets = __h81.freeze(__h81.to_auto(h_start) + __h81.arange(0, __h81.to_auto(BLOCK_H)), _layout=gl.BlockedLayout([4], [64], [4], [0]))
        h_mask = __h81.freeze(__h81.compare(__h81.to_auto(h_offsets), __h81.to_auto(hidden_size), 'lt'), _layout=gl.BlockedLayout([4], [64], [4], [0]))
        acc = __h81.freeze(__h81.zeros([__h81.to_auto(BLOCK_H)], dtype=gl.float32), _layout=gl.BlockedLayout([4], [64], [4], [0]))
        for k in gl.static_range(__h81.to_auto(HC)):
            ...
            rk = __h81.freeze(__h81.load(__h81.to_auto(residual_ptr) + __h81.to_auto(res_base) + __h81.to_auto(k) * __h81.to_auto(res_stride_i) + __h81.to_auto(h_offsets) * __h81.to_auto(res_stride_h), mask=__h81.to_auto(h_mask), other=0.0, _layout=gl.BlockedLayout([4], [64], [4], [0])).to(gl.float32), _layout=gl.BlockedLayout([4], [64], [4], [0]))
            acc = __h81.freeze(__h81.to_auto(acc) + __h81.to_auto(pre_k) * __h81.to_auto(rk), _layout=gl.BlockedLayout([4], [64], [4], [0]))
        __h81.store(__h81.to_auto(layer_input_ptr) + __h81.to_auto(pid_n) * __h81.to_auto(li_stride_n) + __h81.to_auto(h_offsets) * __h81.to_auto(li_stride_h), __h81.to_auto(acc).to(gl.bfloat16), mask=__h81.to_auto(h_mask), _layout=gl.BlockedLayout([4], [64], [4], [0]))
